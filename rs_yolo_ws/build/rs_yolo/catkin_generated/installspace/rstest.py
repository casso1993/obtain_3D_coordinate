...
def publish_image(real_x, real_y, real_z, classification):
    ...
    detect_result.y = real_x
    detect_result.z = real_z
    detect_result.classification = classification

    pub.publish(detect_result)
    rate.sleep()
    rospy.loginfo("x:%d, y:%d,  z:%d,classification:%s", detect_result.x, detect_result.y,  detect_result.z, detect_result.classification)
'''
    ...
    try:
        while True:
            ...
            for i in range(len(camera_xyz_list)):
                publish_image(camera_xyz_list[i][0], camera_xyz_list[i][1], camera_xyz_list[i][2], class_id_list[i])
